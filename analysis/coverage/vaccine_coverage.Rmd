...
#### Table 1 (dose 4 subset): Summary of population
```{r}
...
km_collated = rbind(
  surv_table("dose1", 1, "Dose 1", data_km),
  surv_table("dose2", 2, "Dose 2",  data_km),
  surv_table("dose3", 3, "Dose 3", data_km),
  surv_table("dose4", 4, "Dose 4 (full)", data_km),
  surv_table("dose4", 4, "Dose 4 (subset)", data_km_dose4)
)

## Calculate coverage in total and uncensored populations
collated = data.frame(
  Dose = c("1", "2", "3", "4 (full)", "4 (subset)"),
...
collated$`Cumulative coverage` = c(round(max(subset(km_collated, dose=="Dose 1")$cum.in.floor)*100,1),
                      ...
                      round(max(subset(km_collated, dose=="Dose 4 (full)")$cum.in.floor)*100,1),
                      round(max(subset(km_collated, dose=="Dose 4 (subset)")$cum.in.floor)*100,1)
                      )
...
extract_subset_max = function(selected_subset) {
  data.frame(
    cov = c(round(max(surv_table("dose1", 1, "Dose 1", subset(data_km, ckd_5cat==selected_subset))$cum.in.floor)*100,1),
        round(max(surv_table("dose2", 2, "Dose 2", subset(data_km, ckd_5cat==selected_subset))$cum.in.floor)*100,1),
        round(max(surv_table("dose3", 3, "Dose 3", subset(data_km, ckd_5cat==selected_subset))$cum.in.floor)*100,1),
        round(max(surv_table("dose4", 4, "Dose 4 (full)", subset(data_km, ckd_5cat==selected_subset))$cum.in.floor)*100,1),
        round(max(surv_table("dose4", 4, "Dose 4 (subset)", subset(data_km_dose4, ckd_5cat==selected_subset))$cum.in.floor)*100,1))
  )
}
collated_cov = cbind(
  Dose = c("1", "2", "3", "4 (full)", "4 (subset)"),
...
km_collated$dose_plot[km_collated$dose %in% c("Dose 4 (full)", "Dose 4 (subset)")] = "Dose 4"

# Create variable for full vs subset
km_collated$Population = "Full"
km_collated$Population[km_collated$dose=="Dose 4 (subset)"] = "Subset"

km_plot = ggplot(km_collated, aes(x = date, y = cum.in.floor*100, colour = factor(dose_plot))) + geom_line(aes(linetype=Population)) +
...
plot_reduced_tbl = function(input_tbl, dose4=FALSE, level = c("partial", "full")) {
  ...
  if (dose4==TRUE) { plot_min = 0 } else { plot_min = min(input_tbl$`% vaccinated`, na.rm=T)-10 }
...
plot_reduced_tbl_subgroup = function(input_tbl, dose4=FALSE) {
  ...
  ## Set axis breaks depending on outcome
  if (dose4==TRUE) { 
      g1 = g1 + scale_y_continuous(trans="log",limits=c(plot_min, plot_max), breaks=c(0.25,1,4)) 
  } else { 
      g1 = g1 + scale_y_continuous(trans="log",limits=c(plot_min, plot_max), breaks=c(0.5,1,2)) 
  }
  g1
