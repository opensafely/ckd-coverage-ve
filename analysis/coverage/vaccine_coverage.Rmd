...
#### Table 1: Summary of population, stratified by CKD status
```{r}
...
#### Coverage over time (uncensored population)
...
#### Coverage over time (1 - Kaplan-Meier)
...
data_km <- data_cohort %>%
  mutate(
    ...
    # End date
    end_date = as.Date("2022-02-16", format = "%Y-%m-%d"),
...
# Dose 1
decimal_threshold = 10/nrow(data_km)
surv_table = function(dose, dosenum) {
  surv = survfit(as.formula(paste0("Surv(follow_up_time_",dose,", covid_vax_",dose,") ~ 1")), data = data_km) %>% 
    broom::tidy() %>% 
    filter(estimate > 0) %>%
    mutate(
      estimate_rounded = pmin(1,plyr::round_any(estimate, decimal_threshold), na.rm=TRUE),
      cum.in_rounded = 1 - estimate,
      dose = paste0("dose ",dosenum),
      date = as.Date("2020-12-01", format = "%Y-%m-%d")+time
    ) %>%
    select(time, n.risk, n.censor, cum.in_rounded, dose, date) %>%
    filter(time>=0)
}
km_collated = rbind(
  surv_table("dose1", 1),
...
palette = c("#3B9AB2","#78B7C5","#EBCC2A","#E1AF00","#B40F20")
ggplot(km_collated, aes(x = date, y = cum.in_rounded, colour = factor(dose))) + geom_line() +
          theme_bw()+ scale_colour_manual(values = palette[c(1,2,4,5)]) +
          theme(legend.position="right") + 
  ylim(0,1) + ylab("Vaccination probability") + labs(colour="dose") +
  ggtitle("Any vaccine") +
  theme(legend.title = element_text(size=14), legend.text = element_text(size=12), axis.text = element_text(size=14), axis.title = element_text(size=14)) 
```
...
collated = data.frame(
  Dose = c("1", "2", "3", "4", "4 (subset)"),
  N_cohort_total = c(rep(rounded_n, 4), rounded_n_dose4),
  N_vaccinated_total = c(sum(data_cohort$vax1_date<=end_date, na.rm=T), sum(data_cohort$vax2_date<=end_date, na.rm=T), 
                         ...
                         sum(data_dose4$vax4_date<=end_date, na.rm=T))
) %>%
  mutate(
    coverage_total = round(N_vaccinated_total/N_cohort_total*100,1),
    N_cohort_uncensored = c(rep(rounded_n_uncensored, 4), rounded_n_dose4_uncensored),
    N_vaccinated_uncensored = c(sum(data_uncensored$vax1_date<=end_date, na.rm=T), sum(data_uncensored$vax2_date<=end_date, na.rm=T), 
                                ...
                                sum(data_dose4_uncensored$vax4_date<=end_date, na.rm=T)),
...
collated$`Cumulative probability` = c(round(max(subset(km_collated, dose=="dose 1")$cum.in_rounded)*100,1),
                      round(max(subset(km_collated, dose=="dose 2")$cum.in_rounded)*100,1),
                      round(max(subset(km_collated, dose=="dose 3")$cum.in_rounded)*100,1),
                      round(max(subset(km_collated, dose=="dose 4")$cum.in_rounded)*100,1), "-")
...
gt(collated) %>%
   tab_spanner(
    ...
    tab_spanner(
    label = "KM",
    columns = c("Cumulative probability")
  )
```
<br/>Dose 4 subset calculations restricted to individuals with a history of immunosuppression, haematologic cancer, or transplant on the index date (01-Dec-2020). Uncensored analyses restricted to individauls alive and registered throughout follow-up (01-Dec-2020 to 16-Feb-2022).<br/><br/>  
...
create_reduced_tbl = function(input_tbl) {
  ...
  input_tbl$perc_unvac <- round(as.numeric(input_tbl$n_unvax_at_cut_off)/as.numeric(input_tbl$n_uncensored_at_cut_off)*100,1)
...
dose2_tbl <- read_rds(here::here("output", "model", paste0("mod_strat_coxph_redacted_dose2_full.rds"))) %>%
  create_reduced_tbl()
...
##### Univariate and multivariate model outputs (scroll left/right for full table)
```{r}
dose4full_tbl <- read_rds(here::here("output", "model", paste0("mod_strat_coxph_redacted_dose4full_full.rds"))) %>%
  create_reduced_tbl()
...
##### Univariate and multivariate model outputs (scroll left/right for full table)
```{r}
dose4subset_tbl <- read_rds(here::here("output", "model", paste0("mod_strat_coxph_redacted_dose4subset_full.rds"))) %>%
  create_reduced_tbl()
...
plot_reduced_tbl_subgroup = function(input_tbl) {
  ## Example: input_tbl = tbl_subgroup
