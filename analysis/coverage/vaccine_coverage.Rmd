...
* eGFR<60 in 2 years before 01 Dec 2020 OR present in UK renal registry on index date   
...
extract_subset_max = function(selected_subset) {
  data.frame(
    cov = c(round(max(surv_table("dose1", 1, subset(data_km, ckd_5cat==selected_subset))$cum.in_rounded)*100,1),
        round(max(surv_table("dose2", 2, subset(data_km, ckd_5cat==selected_subset))$cum.in_rounded)*100,1),
        round(max(surv_table("dose3", 3, subset(data_km, ckd_5cat==selected_subset))$cum.in_rounded)*100,1),
        round(max(surv_table("dose4", 4, subset(data_km, ckd_5cat==selected_subset))$cum.in_rounded)*100,1))
  )
}
collated_cov = cbind(
  dose = 1:4,
...
```
<br/>
Data for full cohort (censored/uncensored). Any counts of <=100 grouped as 'other'; all counts rounded to nearest 5; any rounded counts <=10 redacted.
<br/><br/>  
...
#### Plot of partially adjusted model outputs
```{r, fig.width=8, fig.height=12}
plot_reduced_tbl(dose4_tbl, dose4=TRUE)
```
<br/><br/><br/><br/>
