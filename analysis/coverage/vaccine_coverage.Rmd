...
A total of **`r prettyNum(rounded_n)`** (rounded to the nearest 5) met the inclusion criteria (see *output/data/flowchart.csv* for additional details), of which **`r prettyNum(rounded_n_uncensored)`** remained uncensored (alive and registered) throughout follow-up (01-Dec-2020 to 30-Mar-2022).
...
ggplot(km_collated, aes(x = date, y = cum.in_rounded, colour = factor(dose))) + geom_line() +
          theme_bw()+ scale_colour_manual(values = palette[c(1,2,4,5)]) +
          theme(legend.position="right") + 
  ylim(0,1) + ylab("cumulative coverage") + labs(colour="dose") +
  ggtitle("Any vaccine") +
  theme(legend.title = element_text(size=14), legend.text = element_text(size=12), axis.text = element_text(size=14), axis.title = element_text(size=14)) 
```

#### Coverage as of 30-Mar-2022
```{r}
...
collated$`Cumulative coverage` = c(round(max(subset(km_collated, dose=="dose 1")$cum.in_rounded)*100,1),
...
```
<br/>Dose 4 subset calculations restricted to individuals with a history of immunosuppression, haematologic cancer, or transplant on the index date (01-Dec-2020). Uncensored analyses restricted to individauls alive and registered throughout follow-up (01-Dec-2020 to 30-Mar-2022).<br/><br/>  
...
g2 = ggplot(collated, aes(fill=Vaccine, y=as.numeric(Percent), x=Dose)) + 
    geom_bar(position="stack", stat="identity") +
    theme_bw() + ylab("% doses") + scale_fill_manual(values = palette[c(2,1,3)]) +
    theme(legend.position="right") + xlab("Dose") +
...
#### Covariates associated with vaccine coverage: dose 3 by 30 March 2022
...
Cox proportional hazards models were used to explore factors associated with completion of a 2-dose series via any product combination between **08-Dec-2020** and **30-Mar-2022**.  
    
Follow-up was censored on the date of death, deregistration, or the end of the study period (30-Mar-2022). Overall coverage among individuals uncensored on 30-Mar-2022 was `r vax_perc_uncensored`% (`r prettyNum(total_vax_uncensored)` / `r prettyNum(total_N_uncensored)`, rounded to nearest 5). 
<br/><br/>
  ...
  input_tbl %>% 
    ...
    tab_options(table.font.size = pct(85)) %>%
     tab_style(
        ...
        locations = list(
          cells_body(
            columns = c("p (min)", "p (multi)", "% unvaccinated")
          )
        )
     ) %>%
    tab_options(row_group.font.weight="bold", column_labels.font.weigh="bold")
...
names(tbl_reduced) = c("var_group", "group", "level", "n (total)", "n (vaccinated)", "% unvaccinated",
                            "OR (min)", "lower CI (min)", "upper CI (min)", "p (min)",
...
tbl_reduced %>% 
  ...
  tab_spanner(
    label = "minimally adjusted",
    columns = c("OR (min)", "lower CI (min)", "upper CI (min)", "p (min)")
  ) %>%
    tab_spanner(
    label = "partially adjusted",
...
#### Covariates associated with vaccine coverage: dose 4 by 30 Mar 2022 (full cohort)
...
#### Covariates associated with vaccine coverage: dose 4 by 30 Mar 2022 (subset)
...
create_reduced_tbl_subgroup = function(selected_dose=c("dose3", "dose4full", "dose4subset")) {
  # Example: selected_dose = "dose2"
...
## Function to create formatted gt output
tabulate_reduced_tbl_subgroup = function(input_tbl, selected_dose=c("dose3", "dose4full", "dose4subset")) { 
  ...
  tbl_reduced <- input_tbl %>%
    mutate(
      ...
      HR_summary_minimal = paste0(estimate_minimal," (", conf.low,"-",conf.high,")"),
      HR_summary_partial = paste0(estimate_partial," (", conf.low,"-",conf.high,")"),
      HR_summary_full = paste0(estimate_minimal," (", conf.low,"-",conf.high,")"),
      
      # Clean summary outputs
      HR_summary_minimal = ifelse(HR_summary_minimal=="[Redacted] ([Redacted]-[Redacted])", "[Redacted]", HR_summary_minimal),
      ...
      HR_summary_minimal = ifelse(HR_summary_minimal=="1 (NA-NA)", "1", HR_summary_minimal),
    ...
    select(subgroup, subgroup_id, var_group, var_label, label, n_obs, n_uncensored_at_cut_off, perc_unvac,
           HR_summary_minimal, HR_summary_partial, HR_summary_full)
  
  ## Relabel columns
  names(tbl_reduced) = c("subgroup", "subgroup_id", "var_group", "group", "level", "n (total)", "n (uncen)", "% unvax", "HR minimal", "HR partial", "HR full")

  ## Pivot to wide format based on subgroup
  tbl_wide = tbl_reduced %>% 
    pivot_wider(    
      names_from = c(subgroup, subgroup_id),
      values_from = c(`n (total)`, `n (uncen)`, `% unvax`, `HR minimal`, `HR partial`, `HR full`),
  ...
  tbl_wide = tbl_wide %>%
    ...
    tab_spanner(
      label = "CKD3a (D-T-)",
      columns = paste0(c("n (total)", "n (uncen)", "% unvax", "HR minimal", "HR partial", "HR full")," [1]")
    )  %>%
    tab_spanner(
      label = "CKD3b (D-T-)",
      columns = paste0(c("n (total)", "n (uncen)", "% unvax", "HR minimal", "HR partial", "HR full")," [2]")
    )  %>%
    tab_spanner(
      label = "CKD4-5 (D-T-)",
      columns = paste0(c("n (total)", "n (uncen)", "% unvax", "HR minimal", "HR partial", "HR full")," [3]")
    )  %>%
    tab_spanner(
      label = "CKD (T+)",
      columns = paste0(c("n (total)", "n (uncen)", "% unvax", "HR minimal", "HR partial", "HR full")," [4]")
    )  %>%
    cols_hide(columns = "var_group") %>%
    ...
    if (selected_dose != "dose4subset") { 
      tbl_wide = tbl_wide %>%
        tab_spanner(
          label = "CKD (D+T-)",
          columns = paste0(c("n (total)", "n (uncen)", "% unvax", "HR minimal", "HR partial", "HR full")," [5]")
        ) 
      } 
  return(tbl_wide)
}
...
plot_reduced_tbl_subgroup(dose3_tbl_subgroup)
```
<br/><br/><br/><br/>  
