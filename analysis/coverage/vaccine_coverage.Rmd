...
# Dose 1
surv_table = function(dose, dosenum, data_input) {
...
palette = c("#3B9AB2","#78B7C5","#EBCC2A","#E1AF00","#B40F20")
km_plot = ggplot(km_collated, aes(x = date, y = cum.in_rounded*100, colour = factor(dose))) + geom_line() +
          theme_bw()+ scale_colour_manual(values = palette[c(1,2,4,5)]) +
...
```
<br/>Dose 4 subset calculations restricted to individuals with a history of immunosuppression, haematologic cancer, or transplant on the index date (01-Dec-2020). Uncensored analyses restricted to individauls alive and registered throughout follow-up (01-Dec-2020 to 20-Apr-2022).
<br/><br/>
...
extract_subset_max = function(selected_subset) {
  data.frame(
    cov = c(round(max(surv_table("dose1", 1, subset(data_km, ckd_7cat==selected_subset))$cum.in_rounded)*100,1),
        round(max(surv_table("dose2", 2, subset(data_km, ckd_7cat==selected_subset))$cum.in_rounded)*100,1),
        round(max(surv_table("dose3", 3, subset(data_km, ckd_7cat==selected_subset))$cum.in_rounded)*100,1),
        round(max(surv_table("dose4", 4, subset(data_km, ckd_7cat==selected_subset))$cum.in_rounded)*100,1))
  )
}
collated_cov = cbind(
  dose = 1:4,
  extract_subset_max("CKD3a (D-T-)"),
  extract_subset_max("CKD3b (D-T-)"),
  extract_subset_max("CKD4 (D-T-)"),
  extract_subset_max("CKD5 (D-T-)"),
  extract_subset_max("CKD (D+T-)"),
  extract_subset_max("CKD (D-T+)"),
  extract_subset_max("CKD (D+T+)")
)
names(collated_cov) = c("Dose", "CKD3a (D-T-)", "CKD3b (D-T-)", "CKD4 (D-T-)", "CKD5 (D-T-)", "CKD (D+T-)", "CKD (D-T+)", "CKD (D+T+)")
collated_cov %>% gt() %>%
  tab_options(row_group.font.weight="bold", column_labels.font.weigh="bold") %>%
  tab_options(table.font.size = pct(95))
```
...
plot_grid(km_plot, g1, 
          ncol=2,  align="h", axis = c("tb"), rel_widths=c(1.5,1))
```
<br/>Data for full cohort (censored/uncensored).
<br/><br/>
...
plot_reduced_tbl = function(input_tbl, dose4=FALSE) {
  ## Remove redacted components from plot
  input_tbl$`HR (full)`[input_tbl$`HR (full)`=="[Redacted]"] = NA
  input_tbl$`lower CI (full)`[input_tbl$`HR (full)`=="[Redacted]"] = NA
  input_tbl$`upper CI (full)`[input_tbl$`HR (full)`=="[Redacted]"] = NA
  
  ## Recode HRs and CIs as numerics
  input_tbl$`HR (full)` = as.numeric(input_tbl$`HR (full)`)
  input_tbl$`lower CI (full)` = as.numeric(input_tbl$`lower CI (full)`)
  input_tbl$`upper CI (full)` = as.numeric(input_tbl$`upper CI (full)`)
  
  ## Set plot min
  plot_min = 0.5
  if(min(input_tbl$`lower CI (full)`, na.rm=T) < plot_min) { plot_min = min(input_tbl$`lower CI (full)`, na.rm=T) }
  
  ## Set plot max
  plot_max = 2
  if(max(input_tbl$`upper CI (full)`, na.rm=T) > plot_max) { plot_max = max(input_tbl$`upper CI (full)`, na.rm=T) }
  ...
  ## Generate plot of HRs
  g1 = ggplot(input_tbl, aes(y = `HR (full)`, x = level, colour = var_group)) + 
    geom_point(size = 2, alpha=0.8) +
    geom_text(aes(y = plot_min+0.02, label = redaction)) +
    geom_errorbar(aes(ymin=as.numeric(`lower CI (full)`), ymax=as.numeric(`upper CI (full)`), colour = var_group), width=0) + 
    coord_flip() + facet_grid(group~., scales = "free", space = "free") + 
    theme_bw() + theme(strip.background = element_blank()) + labs(colour = "Group") +
    geom_hline(yintercept=1, linetype="dotted") + 
    ylab("Hazard ratio") + xlab("") +
