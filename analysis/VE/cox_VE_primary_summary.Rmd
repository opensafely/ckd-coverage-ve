...
* Registered for at least 3m before first dose    
* Aged ≥16 as of 31-Mar-2021
* eGFR<60 in 2 years before first dose OR present in UK Renal Registry (UKRR) on 31-Dec-2020 (the status update closest to the index date)  
...
* Received first dose after 04-Jan-2021 (the period in which vaccines were in use)
* Dose 1-2 interval was in the range of 8-14 weeks
...
* No SARS-CoV-2 infection between doses 1 and 2 (to mitigate the impact of hybrid immunity)
...
```{r}
flowchart <- read.csv(here::here("output", "tables", "flowchart_VE_primary.csv"))
...
table_1 <- rbind(
  ...
  read_rds(here::here("output", "tables", "table1_VE_primary_redacted_unmatched_RRT.rds")) %>% mutate(subgroup = "RRT (any)", subgroup_id = 5),
  read_rds(here::here("output", "tables", "table1_VE_primary_redacted_unmatched_Tx.rds")) %>% mutate(subgroup = "RRT (Tx)", subgroup_id = 6),
  read_rds(here::here("output", "tables", "table1_VE_primary_redacted_unmatched_dialysis.rds")) %>% mutate(subgroup = "RRT (dialysis)", subgroup_id = 7)
)
...
```
Data are n (%) after rounding to the nearest 5. Any cells with rounded counts or non-counts (relative to the denominator) of >0 and <=10 are redacted. Primary care codes and risk groups are coded by separate binary variables; percentages in these subheadings therefore do not sum to 100. **Prior SARS-CoV-2** was determined based on evidence of a positive SARS-CoV-2 test, COVID-19-related primary care code, COVID-19-related A&E admission, or COVID-19-related hospitalisation as of the first dose. JCVI group was inferred based on age and shielding code lists (to define **Clinically extremely vulnerable** individuals). **Comorbidity count** includes severe obesity, chronic heart disease, diabetes, chronic liver disease, any respiratory disease (including asthma), and chronic neurological disease. CKD, chronic kidney disease; CND, chronic neurological disease; RRT, renal replacement therapy. 
<br/><br/>
...
table_irr_matched <- read_rds(here::here("output", "tables", "table_irr_primary_redacted_matched_all.rds"))
...
## Save collated table
write_csv(table_irr, here::here("output", "tables", "irr_hr_summary_VE_primary.csv"))
...
create_subgroup_table = function(selected_subgroup="CKD3") {
  ...
  table_irr_matched <- read_rds(here::here("output", "tables", paste0("table_irr_primary_redacted_matched_",selected_subgroup,".rds")))
...
## Save collated table
write_csv(collate_subgroups, here::here("output", "tables", "irr_hr_summary_VE_primary_subgroups.csv"))
...
for (i in 1:length(outcome_list_sub)) {
  ...
  all <- read_csv(here::here("output", "model", "VE_primary", glue("modelcox_tidy_full_unmatched_persontime_{selected_outcome}_all.csv"))) %>% filter(model==2) %>% mutate(label="All")
  all_ct <- read_csv(here::here("output", "model", "VE_primary", glue("modelcox_tidy_full_unmatched_calendartime_{selected_outcome}_all.csv"))) %>% filter(model==2) %>% mutate(label="All (CT)")
  ckd3 <- read_csv(here::here("output", "model", "VE_primary", glue("modelcox_tidy_full_unmatched_persontime_{selected_outcome}_CKD3.csv"))) %>% filter(model==2) %>% mutate(label="CKD3")
  ckd4_5 <- read_csv(here::here("output", "model", "VE_primary", glue("modelcox_tidy_full_unmatched_persontime_{selected_outcome}_CKD4-5.csv"))) %>% filter(model==2) %>% mutate(label="CKD4-5")
  rrt_any <- read_csv(here::here("output", "model", "VE_primary", glue("modelcox_tidy_full_unmatched_persontime_{selected_outcome}_RRT.csv"))) %>% filter(model==2) %>% mutate(label="RRT (any)")
  rrt_tx <- read_csv(here::here("output", "model", "VE_primary", glue("modelcox_tidy_full_unmatched_persontime_{selected_outcome}_Tx.csv"))) %>% filter(model==2) %>% mutate(label="RRT (Tx)")
  rrt_dialysis <- read_csv(here::here("output", "model", "VE_primary", glue("modelcox_tidy_full_unmatched_persontime_{selected_outcome}_dialysis.csv"))) %>% filter(model==2) %>% mutate(label="RRT (dialysis)")
  all_strata <- read_csv(here::here("output", "model", "VE_primary", glue("modelcox_tidy_strata_unmatched_persontime_{selected_outcome}_all.csv"))) %>% filter(model==2) %>% mutate(label="All - strata")
  all_ct_strata <- read_csv(here::here("output", "model", "VE_primary", glue("modelcox_tidy_strata_unmatched_calendartime_{selected_outcome}_all.csv"))) %>% filter(model==2) %>% mutate(label="All (CT) - strata")
  ...
  ## Save
  write_csv(merged_wide, here::here("output", "model", "summary", glue("VE_primary_covariate_estimates_{selected_outcome}.csv")))
}
```
.