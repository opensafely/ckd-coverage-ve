...
##### Whole population - modified for publication
```{r fig.width=12, fig.height=10}
set_breaks <- function(x) { 
  if (max(x)<1) { seq(0, 1, 0.25) } 
  else if (max(x)>1 & max(x)<10) { seq(0, 10, 2.5) }
  else { seq(0, 100, 25) }
}

set_limits <- function(x) { 
  if (max(x)<1) { c(0,1) } 
  else if (max(x)>1 & max(x)<10) { c(0,10) }
  else { c(0,100) }
}

generate_km_plot(surv_combined) + 
  scale_y_continuous(expand = expansion(mult=c(0,0.01)), breaks = set_breaks, limits = set_limits) +
