...
dir.create(here::here("output", "model", "VE"), showWarnings = FALSE, recursive=TRUE)
...
cat(glue("## script info for cox models ##"), "  \n", file = here("output", "model", "VE", glue("modelcox_log.txt")), append = FALSE)

## function to pass additional log text
logoutput <- function(...){
  cat(..., file = here("output", "model", "VE", glue("modelcox_log.txt")), sep = "\n  ", append = TRUE)
...
formula2 <- formula1 %>% update(. ~ . + poly(age, degree = 2, raw = TRUE) + sex + imd + ethnicity + rural_urban_group + 
  prior_covid_cat + prevax_tests_cat + learning_disability + sev_mental_ill)
...
formula2_full <- formula1_full %>% update(. ~ . + poly(age, degree = 2, raw = TRUE) + sex + imd + ethnicity + rural_urban_group + 
                                  prior_covid_cat + prevax_tests_cat + learning_disability + sev_mental_ill)
...
irr_table = read_rds(here("output", "tables", "table_irr_redacted.rds"))
...
for (i in 1:length(outcome_list)) {
  ...
  write_csv(model_glance, here::here("output", "model", "VE", glue(paste0("modelcox_glance_",selected_outcome,".csv"))))
  ...
  write_csv(model_tidy_reduced, here::here("output", "model", "VE", glue(paste0("modelcox_tidy_reduced_",selected_outcome,".csv"))))
  write_rds(model_tidy_reduced, here::here("output", "model", "VE", glue(paste0("modelcox_tidy_reduced_",selected_outcome,".rds"))), compress="gz")
}
