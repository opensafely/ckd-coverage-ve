---
title: "Descriptive statistics for follow-up periods in "
...
## Set palette
palette1 = c("#fe9929", "#d95f0e", "#993404")
palette2 = c("#018571", "#a6611a")
...
summary_df$n_enrolled[summary_df$date>as.Date("2021-11-14") | (summary_df$n_enrolled>0 & summary_df$n_enrolled<=redaction_threshold)] = NA
summary_df$cumulative_nextvax[summary_df$date>as.Date("2021-11-14") | (summary_df$cumulative_nextvax>0 & summary_df$cumulative_nextvax<=redaction_threshold)] = NA
summary_df$n_cases[summary_df$date>as.Date("2021-11-14") | (summary_df$n_cases>0 & summary_df$n_cases<=redaction_threshold)] = NA

## Set variant
summary_df$variant = "Delta"
...
g1 = ggplot(summary_df, aes(x=date, y=n_enrolled/1000)) + 
    ...
    geom_vline(xintercept = as.Date(c("2021-01-01", "2021-11-14")), linetype="dotted", colour=palette1[3]) +
...
g2 = ggplot(summary_df, aes(x=date, y=n_cases, fill=variant)) + 
    ...
    geom_vline(xintercept = as.Date(c("2021-01-01", "2021-11-14")), linetype="dotted", colour=palette1[3]) +
    scale_fill_manual(values = c(palette2[1], palette2[2])) +
...
data.frame(
  ...
  mutate(
  `SARS-CoV-2 test` = plyr::round_any(quantile(d$tte_covid_postest_or_censor)[2:4], round_threshold),
  `COVID-19 hosp.` = plyr::round_any(quantile(d$tte_covid_hosp_or_censor)[2:4], round_threshold),
  `COVID-19 death` = plyr::round_any(quantile(d$tte_covid_death_or_censor)[2:4], round_threshold),
  `Non-COVID-19 death` = plyr::round_any(quantile(d$tte_noncovid_death_or_censor)[2:4], round_threshold)
  ) %>% 
  gt()
```
...
#### Uncensored at end of follow-up
```{r}
data.frame(
  Day = c("Day 182")
  ) %>% 
...
## Create summary data frame
summary_df = data.frame(
  date = seq(as.Date("2021-09-01", format="%Y-%m-%d"), as.Date("2022-04-30", format="%Y-%m-%d"), "weeks")
)
...
## Set non-zero values below redaction threshold to 0
summary_df$n_enrolled[summary_df$date>as.Date("2022-04-21") | (summary_df$n_enrolled>0 & summary_df$n_enrolled<=redaction_threshold)] = NA
summary_df$cumulative_nextvax[summary_df$date>as.Date("2022-04-21") | (summary_df$cumulative_nextvax>0 & summary_df$cumulative_nextvax<=redaction_threshold)] = NA
summary_df$n_cases[summary_df$date>as.Date("2022-04-21") | (summary_df$n_cases>0 & summary_df$n_cases<=redaction_threshold)] = NA
...
g1 = ggplot(summary_df, aes(x=date, y=n_enrolled/1000)) + 
    ...
    theme_bw() + xlab(" ") + ylab("N followed up\n(1000s)") + 
    geom_vline(xintercept = as.Date(c("2021-09-01", "2022-04-21")), linetype="dotted", colour=palette1[3]) +
    theme(strip.background = element_blank(), strip.text = element_text(size=12),
           axis.text = element_text(size=12), axis.title = element_text(size=12),
           legend.title = element_text(size=12), legend.text = element_text(size=12))
