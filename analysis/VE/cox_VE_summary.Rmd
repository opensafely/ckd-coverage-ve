...
* Region   
* Age within 3 years    
* Dose 2 date within 3 days    
* Dose 1 date within 7 days    
* JCVI group    
* IMD    
* Ethnicity    
...
A total of **`r prettyNum(rounded_n_unmatched)`** (rounded to the nearest 5) met the inclusion criteria (see *output/data/flowchart.csv* for additional details), of which **`r prettyNum(rounded_n_unmatched)`** were included in the matched subcohort.
<br/><br/>
...
```{r}
table_1 <- rbind(
  read_rds(here::here("output", "tables", "table1_VE_redacted_unmatched_all.rds")) %>% mutate(subgroup = "All", subgroup_id = 1),
  read_rds(here::here("output", "tables", "table1_VE_redacted_unmatched_CKD3.rds")) %>% mutate(subgroup = "CKD3", subgroup_id = 2),
  read_rds(here::here("output", "tables", "table1_VE_redacted_unmatched_CKD4-5.rds")) %>% mutate(subgroup = "CKD4-5", subgroup_id = 3),
  read_rds(here::here("output", "tables", "table1_VE_redacted_unmatched_RRT.rds")) %>% mutate(subgroup = "RRT", subgroup_id = 4),
  read_rds(here::here("output", "tables", "table1_VE_redacted_matched_all.rds")) %>% mutate(subgroup = "Matched (all)", subgroup_id = 5)
)
...
model_persontime = rbind(
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_unmatched_persontime_",outcome_list[1],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[1]),
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_unmatched_persontime_",outcome_list[3],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[3]),
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_unmatched_persontime_",outcome_list[4],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[4]),
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_unmatched_persontime_",outcome_list[5],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[5])
  ) %>% 
  mutate(model_group = ifelse(model_name=="unadjusted", "Person-time (unadjusted)", "Person-time (adjusted)"))

## Read in calendartime models
model_calendartime = rbind(
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_unmatched_calendartime_",outcome_list[1],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[1]),
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_unmatched_calendartime_",outcome_list[3],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[3]),
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_unmatched_calendartime_",outcome_list[4],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[4]),
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_unmatched_calendartime_",outcome_list[5],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[5])
  ) %>% 
  mutate(model_group = "Calendar-time (adjusted)")

## Read in matched models
model_matched = rbind(
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_matched_persontime_",outcome_list[1],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[1]),
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_matched_persontime_",outcome_list[3],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[3]),
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_matched_persontime_",outcome_list[4],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[4]),
  read_rds(here::here("output", "model", paste0("modelcox_tidy_reduced_matched_persontime_",outcome_list[5],"_all.rds"))) %>%
    mutate(outcome_clean = clean_list[5])
  ) %>% 
...
model_subset = subset(model, model_name=="fully adjusted" | model_group=="Matched" | model_group=="Person-time (unadjusted)")
model_subset$model_group = factor(model_subset$model_group, levels = c("Person-time (unadjusted)", "Person-time (adjusted)", "Calendar-time (adjusted)", "Matched"))
...
```
Cumulative incidence = 1 - Kaplan-Meier estimate for probability of survival in unadjusted models. Steps occur every 5 events for disclosure control.
<br/><br/>
