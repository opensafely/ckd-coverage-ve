...
redact_table = function(table_input, multiple=TRUE) {
  if (multiple) {
    if (any(table_input$Freq<1000)) {
      # Sum any combinations with less than or equal to 1000 records
      Other = data.frame(Var1 = "Other", Freq = sum(table_input$Freq[table_input$Freq<1000]))
      table_input = rbind(table_input, Other)
      table_input = subset(table_input, Freq>=1000 | Var1=="Other")
    }
  }
  ...
  table_input$Prop = format(round(table_input$Freq/rounded_n*100,1),nsmall=1)
  table_input$Prop = gsub(" ", "", table_input$Prop, fixed = TRUE) # Remove spaces generated by decimal formatting
  table_input$Prop = gsub("(", " (", table_input$Prop, fixed = TRUE) # Add first space before (
